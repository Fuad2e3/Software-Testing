...
exports.addCompany = (req, res) => {
  const { user_id, company_name, email, contact } = req.body;

  if (!user_id || !company_name) {
    return res.status(400).json({ success: false, message: 'User ID and Company Name are required!' });
  }

  const query = 'INSERT INTO companies (user_id, company_name, email, contact) VALUES (?, ?, ?, ?)';
  db.query(query, [user_id, company_name, email, contact], (err, result) => {
    if (err) {
